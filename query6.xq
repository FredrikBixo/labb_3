...
let $QualifiedCities := doc("mondial.xml")/mondial/country/city[./population[last()]>=100000 and ./../encompassed/@continent = "america"]
...
return <root>{for $Airports in $QualifiedAirport
for $Cities in $QualifiedCities

where $Airports/@city = $Cities/@id

return <Values>{data($Cities/name)},{data($Airports/name)}</Values>}</root>
